...
const Lawyer = ({ lawyer }) => {
    ...
    return (
        <div className="card card-side bg-base-100 shadow-sm border border-gray-200  p-2 md:p-4 lg:p-6 rounded-2xl flex-col sm:flex-row">
            <figure>
                <img className='rounded-xl w-52 h-52' src={image} alt="Lawyers-Image" />
            </figure>
            <div className="card-body text-start">
                <div className='md:flex items-center space-y-2 lg:space-y-0 md:gap-2 text-center'>
                    <h3 className='bg-[#09982F10] text-[#09982F] ext-xs font-medium px-3 py-1 rounded-full'> Available </h3>
                    <h3 className='bg-[#176AE510] text-[#176AE5] text-xs font-medium px-3 lg:px-1.5 xl:px-3 py-2 lg:py-1 xl:py-2 rounded-full'> {experience} </h3>
                </div>
                ...
                <div>
                    <Link to={`/LawyerDetails/${id}`}>
                        <button className="btn btn-outline btn-primary rounded-full w-full">View Details</button>
                    </Link>
                </div>
            </div>
        </div>
